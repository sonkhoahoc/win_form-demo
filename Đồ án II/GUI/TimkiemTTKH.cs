...
{
    public partial class TimkiemTTKH : DevExpress.XtraEditors.XtraForm
    {
        public TimkiemTTKH()
        {
            InitializeComponent();
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            worksheet.Name = "Thongtinkhachhang";

            // Tạo phần đầu

            Range head = worksheet.get_Range("A1", "F1");
            ...
            for (int i = 1; i < dgvTTKH.Columns.Count + 1; i++)
            {
                worksheet.Cells[3, i] = dgvTTKH.Columns[i - 1].HeaderText;
            }
            Range rowHead = worksheet.get_Range("A3", "F3");
            ...
            {
                for (int j = 0; j < dgvTTKH.Columns.Count; j++)
                {
                    worksheet.Cells[i + 4, j + 1] = dgvTTKH.Rows[i].Cells[j].Value.ToString();
                }
            }
            int rowStart = 4;
            int columnStart = 1;
            int rowEnd = rowStart + dgvTTKH.Rows.Count - 1;
            int columnEnd = dgvTTKH.Columns.Count;
            ...
            saveFileDialog.FileName = "Thongtinkhachhang_" + DateTime.Now.Date.ToString("ddMMyyyy") + ".xlsx";
